...
# loss = loss_sum / rkpt_sum 
# # add grads
# grads = torch.autograd.grad(-loss, x)[0] * self.xz_guidance.scale
# variance_t = self.sch_get_var(t) if self.xz_guidance.use_var else 1.0
# x_ = x_ + variance_t * grads
...
# from hmr4d.model.gmd.utils import recover_root_rot_pos, convert_bf3_to_b4f


# class GMDTrajNet(nn.Module):
#     def __init__(
#         self,
#         args,
#         args_clip,
...
#         # self.te_scheduler = DDPMScheduler(**args.ddpmscheduler_cfg)
...
#     def sch_get_scheduler(self):
#         if self.args.scheduler_type == "ddim":
#             return DDIMScheduler(**self.args.scheduler_opt)
#         elif self.args.scheduler_type == "ddpm":
#             return DDPMScheduler(**self.args.scheduler_opt)
#         else:
#             raise NotImplementedError
    
#     def sch_get_var(self, t):
#         if self.args.scheduler_type == "ddim":
#             return self.te_scheduler._get_variance(t, t-1)
#         elif self.args.scheduler_type == "ddpm":
#             return self.te_scheduler._get_variance(t)
...
#         """
#         Generate HumanML-Vec-4 (L, 4) from the given conditions (Text, XZ, etc.)
...
#         x = self.prepare_x(shape=(B, self.unet.in_channels, 224), generator=generator)
...
#                 x_ = scheduler_out.prev_sample

#                 # classifier guidance
#                 if self.xz_guidance.scale > 0 and (self.xz_guidance.t_end <= t <= self.xz_guidance.t_start):
#                     # use the given kpts or the lined traj to guide the reconstruction
#                     traj_as_rkpt = self.xz_guidance.traj_as_rkpt
#                     if traj_as_rkpt:
...
#                     x0_input = self.encode_hmlvec4(convert_bf3_to_b4f(inputs["cond_traj"], pad_f_to=224))
#                     x0_input_mask = convert_bf3_to_b4f(inputs["cond_traj_mask"], pad_f_to=224)
#                     noisy_xprev_input = scheduler.add_noise(
#                         x0_input,
#                         noise=self.randlike_shape(x0_input.shape, generator),
#                         timesteps=torch.tensor(noise_timestep),
#                     )
#                     x_ = x_ * ~x0_input_mask + noisy_xprev_input * x0_input_mask
...
#             if i % 1 == 0:
#                 x0_ = scheduler_out.pred_original_sample
#                 x0_ = self.decode_hmlvec4(x0_).permute(0, 2, 1)[:, :length]  # (B, L, 4)
