...
def plot_floor(data, big_plane=True, select_index=None):
    # Create a floor
    if select_index is not None:
        minx, miny, _ = data[select_index].min(axis=(0, 1))
        maxx, maxy, _ = data[select_index].max(axis=(0, 1))
    else:
    ...
    scale = (1.25*(maxx - minx)/2, 1.25*(maxy - miny)/2, 1)
    scale = (1.5*(maxx - minx)/2, 1.25*(maxy - miny)/2, 1)
...
def show_traj(coords, select_index=None):
    # create the Curve Datablock
    curveData = bpy.data.curves.new('myCurve', type='CURVE')
    curveData.dimensions = '3D'
    curveData.resolution_u = 2

    if select_index is not None and len(select_index) > 1:
        coords = coords[select_index[0] : select_index[-1] + 1]
    ...
    for i, coord in enumerate(coords):
        x, y = coord
        polyline.points[i].co = (x, y, scale/10, 1)

    # # create Object
    curveOB = bpy.data.objects.new('myCurve', curveData)
    curveData.bevel_depth = scale
