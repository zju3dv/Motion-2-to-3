...
def render(npydata, frames_folder, *, mode, faces_path, gt=False,
           ...
           select_index=None, camera_xy=None, camera_rot=None,
           delete=True, accelerator='gpu',device=[0]):
    if init:
    ...
    # in this part
    if mode == "sequence" and select_index is None:
        perc = 0.2
    ...
    # Show the trajectory
    if mode == "sequence":
        show_traj(data.trajectory, select_index)

    # Create a floor
    plot_floor(data.data, big_plane=False, select_index=select_index)

    # initialize the camera
    camera = Camera(first_root=data.get_root(0), mode=mode, is_mesh=is_mesh)

    if mode != "video" and select_index is not None:
        frameidx = select_index
    else:
    ...
    if mode == "sequence":
        camera.update(data.get_mean_root(select_index))

    imported_obj_names = []
    for index, frameidx in enumerate(frameidx):
        print("Rendering frame", index, "-", frameidx)
        ...
        if mode == "video":
            ...
            img_path = f"{img_name}_{index:02}{ext}"
            path = img_path
        ...
        if camera_xy is not None:
            camera.update_xy(camera_xy)
        if camera_rot is not None:
            camera.update_rot(camera_rot)
