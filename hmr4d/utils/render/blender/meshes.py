...
class Meshes:
    def __init__(self, data, *, gt, mode, faces_path, canonicalize, select_index, always_on_floor, oldrender=True, **kwargs):
    ...
    def get_sequence_mat(self, frac):
        ...
        alpha = 0.65
        rgbcolor = (rgbcolor[0], rgbcolor[1], rgbcolor[2], alpha)
    ...
    def get_mean_root(self, select_index=None):
        if select_index is not None:
            return self.data[select_index].mean((0, 1))
        else:
...
def prepare_meshes(data, canonicalize=True, select_index=None, always_on_floor=False):
    if canonicalize:
    ...
    if select_index is not None:
        data[..., 2] -= data[select_index, :, 2].min()
