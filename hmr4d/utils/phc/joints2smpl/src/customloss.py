...
def compute_jitter_LJD(x):
    """Compute jitter for the input tensor (L, J, D)"""
    diff = x[2:] + x[:-2] - 2 * x[1:-1]
    return diff.pow(2).sum(dim=-1).mean(-1)
...
def body_fitting_loss_3d(
    ...
    # Hack
    smooth_loss = compute_jitter_LJD(model_vertices) * smooth_loss_weight
    smooth_loss = smooth_loss.mean(dim=0)
    #####
    ...
    total_loss = (
        joint3d_loss + pose_prior_loss + angle_prior_loss + shape_prior_loss + collision_loss + pose_preserve_loss + smooth_loss
    )
