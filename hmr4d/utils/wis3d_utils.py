...
def add_motion_as_lines(motion, wis3d, name="joints22", skeleton_type="smpl22", const_color=None, offset=0):
    ...
    vertex_colors = vertex_colors.int()
...
def draw_T_w2c(wis3d, T_w2c, name, radius=0.05, all_in_one=True, l=0.5):
    """
    ...
    starts = -T_w2c[:, :3, :3].mT @ T_w2c[:, :3, [3]]
    starts = starts[:, None, :, 0].expand(-1, 3, -1)  #  (L, 3, 1) -> (L, 3, 3)
    vec = T_w2c[:, :3, :3]  # (L, 3, 3)
    ends = starts + vec * l  # (L, 3, 3)
    if all_in_one:
        starts = starts.reshape(-1, 3)  # (L*3, 3)
        ends = ends.reshape(-1, 3)  # (L*3, 3)
        color_tensor = color_tensor[None].expand(T_w2c.size(0), -1, -1).reshape(-1, 3)  # (L*3, 3)
        ...
        for i in range(len(T_w2c)):
            wis3d.set_scene_id(i)
            vertices, faces, vertex_colors = create_skeleton_mesh(
                starts[i], ends[i], radius, color_tensor, resolution=10
            )
            wis3d.add_mesh(vertices, faces, vertex_colors, name=name)
