...
@torch.no_grad()
def compute_camcoord_metrics(batch, pelvis_idxs=[1, 2], fps=30):
    """
    Args:
        batch (dict): {
            "pred_j3d": (..., J, 3) tensor
...
@torch.no_grad()
def compute_global_metrics(batch):
    ...
    if batch.get("valid_mask") is None:
        valid_mask = torch.ones(seq_length, dtype=bool)
    else:
    ...
    # Additional Metrics
    target_trans = target_j3d_glob[:, 0].cpu()[valid_mask]  # (L, 24, 3)
    pred_trans = pred_j3d_glob[:, 0].cpu()[valid_mask]  # (L, 24, 3)
