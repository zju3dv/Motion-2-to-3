...
class MDMZero1to3(MDM):
    def _build_input(self):
        super()._build_input()
        self.embed_input = nn.Linear(self.latent_dim * 2, self.latent_dim)

    def _build_condition(self):
        self.embed_cond = nn.Linear(self.clip_dim + 4, self.latent_dim)

    def mask_cond(self, prompt, cond_x, force_mask=False):
        """_summary_

        Args:
            prompt (_tensor_): [B, 1, d]
            cond_x: [frames, bs, d], another view of motion
        ...
        if force_mask:
            ...
            cond_mask = 1.0 - rearrange(
                (rand_num >= self.cond_mask_prob).float() * (rand_num < 3 * self.cond_mask_prob).float(), "n -> 1 n 1"
            )
            prompt = prompt * prompt_mask
            cond_x = cond_x * cond_mask
            return prompt, cond_x
        else:
            return prompt, cond_x
    ...
    def forward(self, x, timesteps, prompt_latent, d_T, cond_x, length, uncond=False, **kwargs):
        ...
        x = torch.cat([x, cond_x], dim=-1)  # [seqlen, bs, 2*d]
