...
class ControlTransformerDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.c_norm3 = nn.LayerNorm(d_model, eps=layer_norm_eps, **factory_kwargs)
        self.c_dropout1 = nn.Dropout(dropout)
        self.c_dropout2 = nn.Dropout(dropout)
        self.c_dropout3 = nn.Dropout(dropout)
    ...
    def forward(
        ...
        L, B, _, C = tgt.shape
        assert tgt_mask is None, "Not Implemented this for control attention"

        tgt1, tgt2 = tgt[..., 0, :], tgt[..., 1, :]

        x1 = tgt1
        if self.norm_first:
            x1 = x1 + self._sa_block(self.norm1(x1), tgt_mask, tgt_key_padding_mask)
            x1 = x1 + self._mha_block(self.norm2(x1), memory, memory_mask, memory_key_padding_mask)
            x1 = x1 + self._ff_block(self.norm3(x1))
        else:
            x1 = self.norm1(x1 + self._sa_block(x1, tgt_mask, tgt_key_padding_mask))
            x1 = self.norm2(x1 + self._mha_block(x1, memory, memory_mask, memory_key_padding_mask))
            x1 = self.norm3(x1 + self._ff_block(x1))

        x2 = tgt2
        if self.norm_first:
            x2 = x2 + self._csa_block(self.c_norm1(x2), tgt_mask, tgt_key_padding_mask)
            x2 = x2 + self._cmha_block(self.c_norm2(x2), memory, memory_mask, memory_key_padding_mask)
            x2 = x2 + self._cff_block(self.c_norm3(x2))
        else:
            x2 = self.c_norm1(x2 + self._csa_block(x2, tgt_mask, tgt_key_padding_mask))
            x2 = self.c_norm2(x2 + self._cmha_block(x2, memory, memory_mask, memory_key_padding_mask))
            x2 = self.c_norm3(x2 + self._cff_block(x2))
