...
class MVTransformerEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        L, B, V, C = src.shape
        assert src_mask is None, "Not Implemented this for multi-view attention"
        src_key_padding_mask = repeat(src_key_padding_mask, "b l -> b v l", v=V)  # (B, V, L)
        src_key_padding_mask = src_key_padding_mask.reshape(B * V, L)  # (B*V, L)

        x = src
        if self.norm_first:
            x = x + self._mv_attn_block(self.mv_norm(x), src_mask)
            x = rearrange(x, "l b v c -> l (b v) c")
            x = x + self._sa_block(self.norm1(x), src_mask, src_key_padding_mask)
            x = x + self._ff_block(self.norm2(x))
        else:
            x = x + self.mv_norm(self._mv_attn_block(x, src_mask))
            # x = x + self.mv_norm(self._mv_attn_block(x, src_mask))
            x = rearrange(x, "l b v c -> l (b v) c")
